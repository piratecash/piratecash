...
1. Clone the PirateCash Core source code:
    ```shell
    git clone https://github.com/piratecash/piratecash-core
    cd piratecash-core
    ```

2.  Build PirateCash Core:

    Configure and build the headless PirateCash Core binaries as well as the GUI (if Qt is found).
...
## Disable-wallet mode
When the intention is to run only a P2P node without a wallet, PirateCash Core may be
