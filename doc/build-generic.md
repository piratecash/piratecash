...
------------------------------
In the past, the build documentation contained instructions on how to build Cosanta with system-wide installed dependencies
...
---------------------
Cosanta inherited the `depends` folder from Bitcoin, which contains all dependencies required to build Cosanta. These
dependencies must be built before Cosanta can actually be built. To do so, perform the following:
...
not done, build failures might occur when building Cosanta.
