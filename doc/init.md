...
can be found in the contrib/init folder.
    ...
    contrib/init/piratecash.openrc:     OpenRC compatible SysV style init script
    contrib/init/piratecash.openrcconf: OpenRC conf.d file
    contrib/init/piratecash.conf:       Upstart service configuration file
    contrib/init/piratecash.init:       CentOS compatible SysV style init script
...
Running piratecashd as a daemon does not require any manual configuration. You may
set the `rpcauth` setting in the `piratecash.conf` configuration file to override
...
PID file:            `/var/run/piratecashd/piratecashd.pid` (OpenRC and Upstart) or `/run/piratecashd/piratecashd.pid` (systemd)
Lock file:           `/var/lock/subsys/piratecashd` (CentOS)

The PID directory (if applicable) and data directory should both be owned by the
piratecashcore user and group. It is advised for security reasons to make the
configuration file and data directory only readable by the piratecashcore user and
group. Access to piratecash-cli and other piratecashd rpc clients can then be
...
`/etc/piratecashcore/piratecash.conf` with the current systemd, OpenRC, and Upstart init
files out-of-the-box. This is because the command line options specified in the
init files take precedence over the configurations in
`/etc/piratecashcore/piratecash.conf`. However, some init systems have their own
...
Configuration file:  `~/Library/Application Support/PirateCore/piratecash.conf`
Data directory:      `~/Library/Application Support/PirateCore`
Lock file:           `~/Library/Application Support/PirateCore/.lock`
...
Rename piratecash.openrc to piratecashd and drop it in /etc/init.d.  Double
...
Drop piratecash.conf in /etc/init.  Test by running `service piratecashd start`
...
Copy piratecash.init to /etc/init.d/piratecashd. Test by running `service piratecashd start`.

Using this script, you can adjust the path and flags to the piratecashd program by
setting the PIRATED and FLAGS environment variables in the file
