...
The data directory is the default location where the PirateCash Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.piratecashcore/`
macOS    | `$HOME/Library/Application Support/PirateCashCore/`
Windows  | `%APPDATA%\PirateCashCore\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `piratecash.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual PirateCash blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `piratecash.conf`        | User-defined [configuration settings](piratecash-conf.md) for `piratecashd` or `piratecash-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `piratecashd.pid`        | Stores the process ID (PID) of `piratecashd` or `piratecash-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `piratecashd` or `piratecash-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [piratecash.conf](piratecash-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`piratecash-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by PirateCash Core:

Path           | Description | Repository notes
---------------|-------------|-----------------
`banlist.dat`  | Stores the addresses/subnets of banned nodes; completely ignored and superseded by `banlist.json` in 20.0 | [PR #5574](https://github.com/piratecash/piratecash-core/pull/5574)
