...
Gitian is the deterministic build process that is used to build the Cosanta
...
To build Cosanta Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Cosanta Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cosanta/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/cosanta/cosanta-core
...
./bin/gbuild --commit cosanta=${COMMIT} --url cosanta=${URL} ../cosanta-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cosanta=${COMMIT} --url cosanta=${URL} ../cosanta-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cosanta=${COMMIT} --url cosanta=${URL} ../cosanta-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Cosanta git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cosanta-core/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/cosanta/cosanta-detached-sigs.git

BTCPATH=/some/root/path/cosanta-core
SIGPATH=/some/root/path/cosanta-detached-sigs

./bin/gbuild --url cosanta=${BTCPATH},signature=${SIGPATH} ../cosanta-core/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
[cosanta/gitian.sigs](https://github.com/cosanta/gitian.sigs/) repository, or if that's not possible to create a pull
request.
