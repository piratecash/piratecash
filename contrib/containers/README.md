...
Currently two Docker containers exist, `ci` defines how Cosanta's GitLab CI container is built and the `dev` builds on top of the `ci` to provide a containerized development environment that is as close as possible to CI for contributors! See also [Cosanta on Docker Hub](https://hub.docker.com/u/dashpay) i.e. for the [cosantad container](https://hub.docker.com/r/cosanta/cosanta-cored).
...
to run terminal commands from inside the terminal and build Cosanta Core.
