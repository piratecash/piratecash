...
private:
    ...
    void ProcessHeadersMessage(CNode& pfrom, std::deque<CBlockHeader> headers, bool via_compact_block);
...
void PeerManagerImpl::ProcessHeadersMessage(CNode& pfrom, std::deque<CBlockHeader> headers, bool via_compact_block)
{
    const CNetMsgMaker msgMaker(pfrom.GetCommonVersion());
...
bool PeerManagerImpl::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!postponed_headers.empty()) {
        ...
        ProcessHeadersMessage(*pfrom, std::move(postponed_headers), /*via_compact_block=*/false);
    }
