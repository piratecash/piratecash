...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    auto ptemplate = BlockAssembler(*m_node.sporkman, *m_node.govman, *m_node.llmq_ctx, *m_node.evodb, ::ChainstateActive(), *m_node.mempool, Params()).CreateNewBlock(pubKey, nullptr);
