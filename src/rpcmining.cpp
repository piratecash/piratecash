...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    if (pindexBest->nHeight > SPEC_TARGET_FIX){
        nExpectedTime = staking ? (TARGET_SPACING_SPEC * nNetworkWeight / nWeight) : 0;
    }else{
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Piratecash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Piratecash is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Piratecash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Piratecash is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (lpval.type() != null_type)
    {
        ...
        else
        {
            ...
            hashWatchedChain = hashBestChain;
        ...
        LEAVE_CRITICAL_SECTION(cs_main);
        {
            ...
            while (hashBestChain == hashWatchedChain && IsRPCRunning())
            {
    ...
    result.push_back(Pair("longpollid", hashBestChain.GetHex() + i64tostr(nTransactionsUpdatedLast)));
