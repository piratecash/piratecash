...
static const uint32_t WALLET_PIRATE_KB_TO_CORSAR_B = COIN / 1000; // 1 corsar / B = 0.00001 PIRATE / kB
...
static void SetFeeEstimateMode(const CWallet* pwallet, CCoinControl& cc, const UniValue& estimate_mode, const UniValue& estimate_param)
{
    ...
    if (cc.m_fee_mode == FeeEstimateMode::DASH_KB || cc.m_fee_mode == FeeEstimateMode::DUFF_B) {
        ...
        if (cc.m_fee_mode == FeeEstimateMode::DUFF_B) {
            fee_rate /= WALLET_PIRATE_KB_TO_CORSAR_B;
        }
...
static UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    // With the v19 NodeContext refactor connman is not directly reachable from
    // a wallet RPC handler. interfaces::Chain exposes p2pEnabled(), which is
    // true while P2P networking is up, and mnsync covers the "no peers yet" case.
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"listunspent",
        ...
        "Optionally filter to only include txouts paid to specified addresses.\n",
        {
            ...
            {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "The PirateCash addresses to filter",
                {
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The PirateCash address to receive the change"},
    ...
    RPCHelpMan{"walletcreatefundedpsbt",
        ...
        "Implements the Creator and Updater roles.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                {
                    {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The PirateCash address to receive the change"},
                    {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                    {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                    {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
                    {"feeRate", RPCArg::Type::AMOUNT, /* default */ "not set: makes wallet determine the fee", "Set a specific fee rate in " + CURRENCY_UNIT + "/kB"},
