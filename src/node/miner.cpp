...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, std::shared_ptr<wallet::CWallet> pwallet, int64_t block_time, bool isPos)
{
    ...
    UniqueLock lock_main(::cs_main, "::cs_main", __FILE__, __LINE__);
    ...
    m_lock_time_cutoff = nMedianTimePast;
    ...
    if (!skip_pos_template_validation && !TestBlockValidity(state, m_chainlocks, m_evoDb, chainparams, m_chainstate, *pblock, pindexPrev, false, false)) {
        if (isPos) {
...
void PoSMiner(std::shared_ptr<wallet::CWallet> pwallet, NodeContext& node, CThreadInterrupt& interrupt)
{
    ...
    BlockAssembler ba{Assert(node.chainman)->ActiveChainstate(), node, Assert(node.mempool.get()), Params()};
