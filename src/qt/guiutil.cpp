...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("piratecash"))
        return false;
...
{
    // Convert piratecash:// to piratecash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("piratecash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "piratecash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Piratecash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Piratecash.lnk
...
{
    return GetAutostartDir() / "piratecash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a piratecash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Piratecash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the piratecash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add piratecash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Piratecash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  piratecash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Piratecash-Qt"));
