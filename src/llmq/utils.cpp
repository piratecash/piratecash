                                                        ...
                                                        const PreviousQuorumQuarters& previousQuarters)
{
    ...
    const bool use_v18_picking =
        util_params.m_chainman.GetParams().NetworkIDString() == CBaseChainParams::MAIN
    ...
    if (use_v18_picking) {
        size_t firstSkippedIndex_v18 = 0;
        size_t idx_v18{0};
        for (size_t i = 0; i < nQuorums; ++i) {
            ...
            while (quarterQuorumMembers[i].size() < quarterSize && (usedMNsCount + quarterQuorumMembers[i].size() < sortedCombinedMnsList.size())) {
                ...
                if (++idx_v18 == sortedCombinedMnsList.size()) {
                    idx_v18 = 0;
                }
            }
        }
