 ...
 *  Note that -reindex-chainstate skips the validation that happens here!
 */
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, bool fCheckProof = true) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    (void)fCheckProof;
    AssertLockHeld(::cs_main);
...
bool ChainstateManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, const CChain* active_chain)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(block, state, m_blockman, chainparams, pindexPrev, GetAdjustedTime(), true, active_chain ? active_chain : &ActiveChain())) {
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader* first_invalid)
{
...

    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            bool accepted{AcceptBlockHeader(header, state, chainparams, &pindex, &ActiveChain())};
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    bool accepted_header{m_chainman.AcceptBlockHeader(block, state, m_params, &pindex, &m_chain)};
    CheckBlockIndex();
