...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, const CChain* active_chain)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!ContextualCheckBlockHeader(block, state, *this, chainparams, pindexPrev, GetAdjustedTime(), true, active_chain)) {
...
bool ChainstateManager::ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader* first_invalid)
{
...

    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            bool accepted = m_blockman.AcceptBlockHeader(header, state, chainparams, &pindex, &ActiveChain());
